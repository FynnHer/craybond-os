...
typedef struct {
    ...
    uint8_t revision;           // Revision (0 for ACPI 1.0, 2 for ACPI 2.0+)
    uint32_t rsdt_address;      // ACPI 1.0 addr
    // ACPI 2.0+:
    uint32_t length;            // Total size of RSDP (should be at least 36)
    uint64_t xsdt_address;      // 64-bit address of XSDT (new table)
    uint8_t extended_checksum;  // Checksum of the entire table
    uint8_t reserved[3];
